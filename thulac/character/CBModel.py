...
class CBModel:
    ...
    def average(self, step):
        for i in range(self.l_size * self.f_size):
            self.fl_weights[i] = int(
                (float(self.fl_weights[i]) - self.ave_fl_weights[i] / float(step)) * self.DEC + 0.5)

        for i in range(self.l_size * self.l_size):
            self.ll_weights[i] = int(
                (float(self.ll_weights[i]) - self.ave_ll_weights[i] / float(step)) * self.DEC + 0.5)
