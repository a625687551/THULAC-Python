...
class CBNGramFeature:
    ...
    def putValues(self, sequence, mylen, values):
        ...
        self.uniBases[mylen], self.biBases[mylen + 1] = self.findBases(self.datSize, sequence[mylen - 1],
                                                                       self.SENTENCE_BOUNDARY)

        self.uniBases[mylen + 1], self.biBases[mylen + 2] = self.findBases(self.datSize, self.SENTENCE_BOUNDARY,
                                                                           self.SENTENCE_BOUNDARY)
